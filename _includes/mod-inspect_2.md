...
{%- assign _keys = include.val.keys %}
{%- if _keys %}{%- elsif include.val.size %}
{%- assign _keys = "" | split: "," %}
{%- for v in include.val %}
{%- assign _keys = _keys | push: v[0] %}
{%- endfor %}
{%- endif %}
{%- if include.sort != false and _keys %}{%- assign _keys = _keys | sort %}{%- endif %}

{%- if include.include == 'ALL' %} {%- assign _include = _keys %} {%- endif %}
...
{{ _tab }} {%- else -%} {{- '' }}
{%- endif %}
{%- include mod-label_2.md text=key pad=_pad %}{{-' : '-}}
{%- if _blok contains key -%} [{%- include mod-plural_2.md val=_val word=word_key %}]
{%- elsif _bloc contains key -%} [{%- include mod-plural_2.md val=_val.keys word=word_key %}]
{%- elsif _blob contains key -%} [{%- include mod-plural_2.md val=_val word=word_character %}]
{%- elsif _bloi contains key -%} [{%- include mod-plural_2.md val=_val %}]
